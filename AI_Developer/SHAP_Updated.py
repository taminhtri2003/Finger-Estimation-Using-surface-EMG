...
# Identify top 4 sensors based on mean SHAP values (to confirm with bar plot)
sensor_importance = np.mean(np.abs(mean_sensor_shap), axis=0)
sensor_importance_df = pd.DataFrame({
    'Sensor': sensor_names,
    'Mean_SHAP': sensor_importance
}).sort_values('Mean_SHAP', ascending=False)
top_4_sensors = sensor_importance_df['Sensor'].iloc[:4].tolist()  # ['APL', 'EI', 'FDP', 'ED']
top_4_indices = [sensor_names.index(sensor) for sensor in top_4_sensors]

# Filter mean_sensor_shap for top 4 sensors
mean_sensor_shap_top4 = mean_sensor_shap[:, top_4_indices]
sensor_names_top4 = top_4_sensors

# 1. Bar Plot - Global Sensor Importance (already shows top 4 naturally)
...
# 2. Violin Plot - Distribution of Sensor Contributions (Top 4)
plt.figure(figsize=(12, 6))
shap.summary_plot(mean_sensor_shap_top4, feature_names=sensor_names_top4, plot_type="violin", show=False)
plt.title('Distribution of Top 4 Sensor Contributions (Violin Plot)')
plt.tight_layout()
plt.show()

# 3. Waterfall Plot - Sensor Contributions for a Single Prediction (Top 4)
...
sensor_shap_top4 = sensor_shap[top_4_indices]

plt.figure(figsize=(12, 6))
shap.waterfall_plot(
    shap.Explanation(
        values=sensor_shap_top4,
        base_values=explainer.expected_value,
        feature_names=sensor_names_top4
    ),
    max_display=4
)
plt.title('Waterfall Plot - Top 4 Sensor Contributions for Joint 1, First Sample')
plt.tight_layout()
plt.show()

# 4. Decision Plot - Cumulative Sensor Impact (Top 4)
plt.figure(figsize=(12, 6))
shap.decision_plot(
    explainer.expected_value,
    mean_sensor_shap_top4,
    feature_names=sensor_names_top4,
    highlight=[0],  # Highlight first sample
    show=False
)
plt.title('Decision Plot - Cumulative Impact of Top 4 Sensors Across Samples')
plt.tight_layout()
plt.show()

# 5. Custom Embedding Plot - Sensor Contributions with t-SNE (Top 4)
tsne = TSNE(n_components=2, random_state=42, perplexity=min(5, X_test.shape[0]-1))
embedding = tsne.fit_transform(mean_sensor_shap_top4)
plt.figure(figsize=(12, 8))
scatter = plt.scatter(embedding[:, 0], embedding[:, 1], c=np.sum(mean_sensor_shap_top4, axis=1), cmap='viridis')
plt.colorbar(scatter, label='Total SHAP Contribution')
for i, name in enumerate(sensor_names_top4):
    plt.annotate(name, (np.mean(embedding[mean_sensor_shap_top4[:, i] > 0, 0]), 
                       np.mean(embedding[mean_sensor_shap_top4[:, i] > 0, 1])), 
                 alpha=0.7)
plt.title('t-SNE Embedding of Top 4 Sensor Contributions')
plt.xlabel('t-SNE Component 1')
plt.ylabel('t-SNE Component 2')
plt.tight_layout()
plt.show()

# 6. Custom Heatmap - Sensor Contributions Across Samples (Top 4)
plt.figure(figsize=(12, 8))
sns.heatmap(mean_sensor_shap_top4.T, cmap='viridis', xticklabels=True, yticklabels=sensor_names_top4)
plt.xlabel('Test Sample Index')
plt.ylabel('Top 4 Sensors')
plt.title('Heatmap - Top 4 Sensor Contributions Across Samples')
plt.tight_layout()
plt.show()

# 7. Contribution Stacked Area Plot - Sensor Impact Over Samples (Top 4)
plt.figure(figsize=(12, 6))
for sensor_idx in range(len(top_4_sensors)):
    plt.fill_between(range(X_test.shape[0]), 
                     np.cumsum(np.abs(mean_sensor_shap_top4[:, :sensor_idx+1]), axis=1)[:, sensor_idx],
                     label=sensor_names_top4[sensor_idx])
plt.xlabel('Test Sample Index')
plt.ylabel('Cumulative Absolute SHAP Value')
plt.title('Stacked Area Plot - Top 4 Sensor Contributions Across Samples')
plt.legend()
plt.tight_layout()
plt.show()

# 8. Pie Chart - Relative Sensor Importance (Top 4)
plt.figure(figsize=(10, 10))
plt.pie(np.mean(np.abs(mean_sensor_shap_top4), axis=0), labels=sensor_names_top4, autopct='%1.1f%%', startangle=90)
plt.title('Pie Chart - Relative Importance of Top 4 Sensors')
plt.axis('equal')
plt.tight_layout()
plt.show()
