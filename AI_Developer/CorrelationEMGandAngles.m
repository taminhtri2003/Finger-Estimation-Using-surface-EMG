% Load the .mat file
data = load('s4_full.mat'); % Replace 'your_data_file.mat' with the actual file name
...
for trial = 1:size(data.dsfilt_emg, 1)
    for task = 1:size(data.dsfilt_emg, 2)
        ...
        hold on;
        time = (1:size(emg_data, 1)) / 200;   % Assuming sampling rate was downsampled to 1kHz, create a time vector in seconds
