...
%   Modified to align with the joint angles defined in the specified article.
%
...
%                        The order of angles is now:
%                        [ThumbCMC, ThumbMCP, ThumbIP, IndexMCP, IndexPIP, IndexDIP,
%                         MiddleMCP, MiddlePIP, MiddleDIP, RingMCP, RingPIP, RingDIP, LittleMCP, LittlePIP, LittleDIP]
    ...
    for trial = 1:size(finger_kinematics, 1)
        for task = 1:size(finger_kinematics, 2)
            ...
            joint_angles = zeros(num_timepoints, 15); % 15 angles total, added ThumbCMC
            % --- Thumb (3 angles) --- % Modified to include ThumbCMC
            % Angle 1: Thumb CMC (Carpometacarpal) 21-20 to 20-17
            joint_angles(:, 1) = calculate_angle_degrees(kinematics_data, 21, 20, 17);
            % Angle 2: Thumb MCP (Metacarpophalangeal)  20-17 to 17-18
            joint_angles(:, 2) = calculate_angle_degrees(kinematics_data, 20, 17, 18);
            % Angle 3: Thumb IP (Interphalangeal) 17-18 to 18-19
            joint_angles(:, 3) = calculate_angle_degrees(kinematics_data, 17, 18, 19);
            % --- Index (3 angles) ---
            % Angle 4: Index MCP 20-1 to 1-5
            joint_angles(:, 4) = calculate_angle_degrees(kinematics_data, 20, 1, 5);
            % Angle 5: Index PIP (Proximal Interphalangeal) 1-5 to 5-6
            joint_angles(:, 5) = calculate_angle_degrees(kinematics_data, 1, 5, 6);
            % Angle 6: Index DIP (Distal Interphalangeal) 5-6 to 6-7
            joint_angles(:, 6) = calculate_angle_degrees(kinematics_data, 5, 6, 7);
            % --- Middle (3 angles) ---
            % Angle 7: Middle MCP 20-2 to 2-8
            joint_angles(:, 7) = calculate_angle_degrees(kinematics_data, 20, 2, 8);
            % Angle 8: Middle PIP 2-8 to 8-9
            joint_angles(:, 8) = calculate_angle_degrees(kinematics_data, 2, 8, 9);
            % Angle 9: Middle DIP 8-9 to 9-10
            joint_angles(:, 9) = calculate_angle_degrees(kinematics_data, 8, 9, 10);
            % --- Ring (3 angles) ---
            % Angle 10: Ring MCP 20-3 to 3-11
            joint_angles(:, 10) = calculate_angle_degrees(kinematics_data, 20, 3, 11);
            % Angle 11: Ring PIP 3-11 to 11-12
            joint_angles(:, 11) = calculate_angle_degrees(kinematics_data, 3, 11, 12);
            % Angle 12: Ring DIP 11-12 to 12-13
            joint_angles(:, 12) = calculate_angle_degrees(kinematics_data, 11, 12, 13);
            % --- Little (3 angles) ---
            % Angle 13: Little MCP 20-4 to 4-14
            joint_angles(:, 13) = calculate_angle_degrees(kinematics_data, 20, 4, 14);
            % Angle 14: Little PIP 4-14 to 14-15
            joint_angles(:, 14) = calculate_angle_degrees(kinematics_data, 4, 14, 15);
            % Angle 15: Little DIP 14-15 to 15-16
            joint_angles(:, 15) = calculate_angle_degrees(kinematics_data, 14, 15, 16);
