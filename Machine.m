...
sample_feature_indices = zeros(1, 4 * 4);  % 4 features x 4 channels
for ch = 1:4
    for f = 1:4
        idx = (ch-1) * num_features_per_channel + f;
        sample_feature_indices((ch-1)*4 + f) = idx;
    end
end
    ...
    for m = 1:length(regression_methods)
        ...
        switch method
            case 'Linear'
                ...
                mdl = fitlm(X_train_norm, Y_train_group);
                
                % Make predictions
                Y_pred_group = predict(mdl, X_test_norm);
